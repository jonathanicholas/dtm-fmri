...
%
function [symbolic_data, pointers] =  sax_modified(data, N, n, alphabet_size, NR_opt, full_data)
...
for i = 1 : length(data) - (N -1)                                       
    ...
    %xxx = mean(full_data);
    %yyy = std(full_data);
    %sub_section = (sub_section - xxx/yyy);
    sub_section = (sub_section - mean(full_data))/std(full_data);     
